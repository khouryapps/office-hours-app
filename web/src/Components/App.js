...
import { Layout, Menu, Icon, Spin, Divider, Form, message, Modal, List, InputNumber, Alert, Tabs, Select } from 'antd';
...
class App extends LoggedInComponent {
  ...
  componentDidMount() {
    ...
    setInterval(() => this.checkVersion(), 1000 * 60 * 60);
  }
  ...
  render() {
    ...
    const { enable2fa_visible, disable2fa_visible, loading_message, error_version, error_version_dismissed } = this.state;
      ...
      return (
          <ErrorBoundary {...baseProps} key="error">
            { error_version && !error_version_dismissed ? (
                <Modal title="Updated Khoury Admin site available" visible={true} maskClosable={false} closable={false} okText="Refresh" cancelText="Continue" onOk={() => window.location.reload()} onCancel={() => this.setState({ error_version_dismissed: true })} >
